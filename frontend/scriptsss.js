...
async function fetchCollection(id) {
  try {
    const response = await fetch(
      `https://edushare.codebloop.my.id/api/v1/auth/collect/${id}`
    );
    if (!response.ok) {
...
async function createOrder() {
  ...
  try {
    const response = await fetch(
      "https://edushare.codebloop.my.id/api/v1/auth/order",
      {
        method: "POST",
        headers: {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  if (predictButton && form) {
    predictButton.addEventListener("click", async function (event) {
      ...
      try {
        const response = await fetch(
          "https://edushare.codebloop.my.id/api/v1/auth/order/Predict",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
